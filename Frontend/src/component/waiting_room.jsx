...
export default function WaitingRoom({socket}) {
  ...
  useEffect(() => {
    const handleUpdatePlayerList = (players) => {
      ...
      const newTeamA = names.slice(0, half).map(makePlayerObj);
      const newTeamB = names.slice(half).map(makePlayerObj);
    ...
    if (!joinedRef.current) {
        socket.emit('joinRoom', { roomCode: lobbyDetails.lobbyCode, player: playerName });
  ...
  const handleSendMessage = () => {
    ...
    if (!text) return;
    socket?.emit('sendMessage', { roomCode: lobbyDetails.lobbyCode, player: playerName, text });
    setNewMessage('');
  };
  ...
  const toggleReady = () => {
    ...
    setTeamA(prev => prev.map(p => p.name === playerName ? { ...p, ready: newReady } : p));
    setTeamB(prev => prev.map(p => p.name === playerName ? { ...p, ready: newReady } : p));

    socket?.emit('setReady', { roomCode: lobbyDetails.lobbyCode, player: playerName, ready: newReady });
  };
  ...
  useEffect(() => {
    ...
    const handleStartBattle = () => {
      Navigate('/battle', { state: { lobby: lobbyDetails, player: playerName } });
    };
          ...
          <div style={styles.card}>
            <h2 style={styles.cardTitle}>
              ...
              <span style={{ fontSize: '14px', color: '#718096', fontWeight: 'normal' }}>
                {lobbyDetails.teamSize} • {lobbyDetails.mode} • {lobbyDetails.difficulty}
              </span>
            </h2>
        ...
        <div style={styles.rightPanel}>
          <div style={styles.card}>
            ...
            <div style={styles.infoCard}>
              ...
              <div style={styles.infoValue}>{lobbyDetails.mode}</div>
            </div>

            <div style={styles.infoCard}>
              <div style={styles.infoLabel}>Difficulty</div>
              <div style={styles.infoValue}>{lobbyDetails.difficulty}</div>
            </div>
            ...
            {allReady && (
              ...
              onClick={() => socket?.emit('startBattle', { roomCode: lobbyDetails.lobbyCode })}>
                🚀 Start Battle
              </button>
            )}
          </div>
